...
final class Municipios extends AbstractDBConnection implements Model
{
    ...
    public function getUsuario(): ?Usuarios
    {
        if (!empty($this->Empresa_id)) {
            return Usuarios::searchForId($this->Empresa_id) ?? new Usuarios();
        }
        return null;
    }
    ...
    {
        return Municipios::search("SELECT * FROM bdentrepalos.municipio");
    }
    ...
    public static function searchForId(int $id): ?Municipios
    {
        try {
            if ($id > 0) {
                ...
                $tmpMun->Connect();
                $getrow = $tmpMun->getRow("SELECT * FROM municipio WHERE id =?", array($id));
                $tmpMun->Disconnect();
